...
func (m *Model) handleKeys(msg tea.KeyMsg) (tea.Cmd, bool) {
	...
	case "i":
		return func() tea.Msg {
			return shared.NavigateMsg{Screen: shared.ScreenPackages, ServerID: m.ctx.ServerID}
		}, true
	case "y":
		return func() tea.Msg {
...
func (m *Model) runWebPanel(action string) tea.Cmd {
	...
	go func() {
		...
		m.ctx.Pool.Disconnect(srv.ID)
		_, err := m.ctx.Pool.ConnectWithRetry(srv.ID, cfg, 4)
		if err != nil {
