...
func (h *handler) registerNode(mux *http.ServeMux) {
	...
	mux.HandleFunc("POST /settings/users/{id}/2fa/reset", adminz(h.postSettingsUserReset2FA))
	mux.HandleFunc("POST /settings/logs", adminz(h.postSettingsLogs))
	mux.HandleFunc("POST /settings/2fa/start", authz(h.postSettings2FAStart))
	mux.HandleFunc("POST /settings/2fa/confirm", authz(h.postSettings2FAConfirm))
